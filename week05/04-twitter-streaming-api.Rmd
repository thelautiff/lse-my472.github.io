...
twitter_token <- create_token(app = "MY572", 
...
```{r}
stream_tweets(q = "", timeout = 30, parse = FALSE, file_name='streamed_tweets.json')
#parse = false means just download it don't try to read it
...
#^ tries to parse stream - from rtweet - parse stream isn't very good - if it doesn't work, we run this second function we took from above.
head(sample_tweets)
...
```{r}
keywords_tweets <- stream_tweets("BTS AND Biden", timeout = 5) # with a shorter timeout there's a lower likelihood of this failing
...
```{r}
stream_tweets(q = c(-125, 26, -65, 49), timeout = 30, file_name='geotweets2.json')
#stream_tweets(q = lookup_coords("usa"), timeout = 30, file_name='__________________')

geo_tweets <- tryCatch({parse_stream('geotweets2.json')},
                       error = function(e)
                           {print(paste("Retrying with alternative function after initial error when parsing file",
                                        'geotweets2.json'));
                           return(recover_stream('geotweets2.json'))})
```
