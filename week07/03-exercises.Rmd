...
library(readtext)
library(tidyverse)
sample_text <- readtext("uol.txt") %>% .[,2]

str_view_all(sample_text, '\\w*\\s*\\d\\w\\w')
...
str_view_all(sample_text, '\\d\\w\\w\\n')


str_replace(sample_text, '(\\d)\\w\\w\\n', "\\1")

# Apparently you can use brackets to get a capture group, and use \\1 to refer to your first capture group.
...
```{r}
my_dfm <- data_char_ukimmig2010 %>%                        # 1. study the data
  corpus() %>% 
