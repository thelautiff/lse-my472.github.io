...
ipu_clean <- ip_and_unemployment %>%
  pivot_wider(c(country, date),
              names_from = series, values_from = value)  # long to wide

# France
ipu_clean %>%
  filter(country == "france") %>%
  filter(unemployment == max(unemployment) | ip == min(ip, na.rm = TRUE)) 
...
ipu_clean %>%
  ...
  summarise(nonNA_ip = sum(!is.na(ip)),
            nonNA_ue = sum(!is.na(unemployment)),
            nonNA_ip_pct = nonNA_ip / length(country),
            nonNA_ue_pct = nonNA_ue / length(country))
...
ipu_clean %>%
  mutate(ip_sq = ip^2,
         ...
         ip_unemployment = ip_sq * log_unemployment) %>%
  head()
