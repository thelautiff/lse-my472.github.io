...
unemp <- unemp %>% mutate(date = as_date(date))

# Create a line-plot with unemployment and date

p1 <- ggplot(unemp) + geom_line(aes(date, unemp))
...
p2 <- p1 + scale_y_continuous(labels = scales::percent) + scale_x_date(limits = xrange, expand = c(0,0)) 


p3 <- p2 + geom_rect(data = pres, aes(ymin = 0, ymax = 0.12, xmin = start, xmax = end, fill = party), alpha = 0.2)

p3

# Adding president names

p4 <- p3 + geom_text(data = pres, aes(x=start, y = yrange[2]), label = name, angle = 90, vjust = 1, hjust = 0.5, size = 2.5)
p4
# Adding lines to separate presidencies

p5 <- p4 + geom_vline(data = pres, aes(xintercept=as.numeric(start)), linetype = 5, color = 'grey60')
p5
